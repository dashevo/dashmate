...
DASHPAY_CONTRACT_ID=$($DASHMATE_BIN config:get --config="${CONFIG}_1" platform.dashpay.contract.id)
DASHPAY_CONTRACT_BLOCK_HEIGHT=$($DASHMATE_BIN config:get --config="${CONFIG}_1" platform.dashpay.contract.blockHeight)
DASHPAY_TOP_LEVEL_IDENTITY_ID=$($DASHMATE_BIN config:get --config="${CONFIG}_1" platform.dashpay.ownerId)
DASHPAY_TOP_LEVEL_IDENTITY_PRIVATE_KEY=$(grep -m 1 "HD private key:" ${PATH_TO_PROJECT_ROOT}/setup.log | awk '{$1=""; printf $5}')
...
DASHPAY_CONTRACT_ID=${DASHPAY_CONTRACT_ID}
DASHPAY_CONTRACT_BLOCK_HEIGHT=${DASHPAY_CONTRACT_BLOCK_HEIGHT}
DASHPAY_TOP_LEVEL_IDENTITY_ID=${DASHPAY_TOP_LEVEL_IDENTITY_ID}
DASHPAY_TOP_LEVEL_IDENTITY_PRIVATE_KEY=${DASHPAY_TOP_LEVEL_IDENTITY_PRIVATE_KEY}
